...
def find_paths(dataset):
    if dataset in ["BTC-USD", "ETH-USD", "XRP-USD"]:
        ...
        read_path = (
            DATA_FOLDER + "raw/" + "stock_market_index/" + "{}.csv".format(dataset)
        )
        save_path = (
            DATA_FOLDER + "processed/" + "stock_market_index/" + "{}/".format(dataset)
        )

    else:
        raise ValueError(
            "Unexpected 'dataset' argument: 'dataset' should be in ['BTC-USD', 'ETH-USD', 'XRP-USD', 'LBMA-GOLD', 'NYMEX-NG', 'OPEC-ORB', 'SP500', 'CAC40', 'SMI']."
        )
...
def load_data(path, dataset):
    ...
    # Data from: https://data.nasdaq.com/data/LBMA/GOLD-gold-price-london-fixing
        ...
        df.drop(
            columns=["USD (PM)", "GBP (AM)", "GBP (PM)", "EURO (AM)", "EURO (PM)"],
            inplace=True,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process data for the given dataset, i.e., load the data, compute the Log-Returns, split the data in train and test set, create sequences with the given number of lags and finally save the sequences."
    )
    parser.add_argument(
        "dataset",
        metavar="dataset",
        type=str,
        help="A string defining the name of the dataset to process.",
    )
    parser.add_argument(
        "nb_lags",
        metavar="nb_lags",
        type=int,
        help="An int defining the number of lags.",
    )
    parser.add_argument(
        "train_ratio",
        metavar="train_ratio",
        type=float,
        help="A float defining the train ratio.",
    )
