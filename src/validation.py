...
def main(model_type, dataset):
    ...
    if model_type in ["NN", "CNN", "LSTM"]:
        ...
        print()
        print(
            "The best performances of {} on {} are achieved when lr = {:.4f}.".format(
                model_type, dataset, best_lr
            )
        )
    ...
    # Find best number of max features for random forest model:
        ...
        best_max_features = find_best_max_features(
            train_lags, train_target, val_lags, val_target, n_periods
        )
        print()
        print(
            "The best performances of {} on {} are achieved when max_features = {:.2f}.".format(
                model_type, dataset, best_max_features
            )
        )
        ...
        raise ValueError(
            "Unexpected 'model_type' argument: 'model_type' should be in ['NN', 'CNN', 'LSTM', 'RandomForest']."
        )


def find_best_lr(
    model_type, nb_lags, train_lags, train_target, val_lags, val_target, n_periods
):
    best_hit_rate = 0

    # Search the best learning rate on validation set:
    for lr in np.concatenate(
        (
            np.linspace(start=1e-4, stop=1e-3, num=9, endpoint=False),
            np.linspace(start=1e-3, stop=1e-2, num=10),
        )
    ):
        # Instantiate model:
        if model_type == "NN":
...
def find_best_max_features(train_lags, train_target, val_lags, val_target, n_periods):
    ...
    train_lags, train_target = (
        train_lags.to("cpu").numpy(),
        train_target.to("cpu").numpy(),
    )
    ...
    for max_features in np.linspace(start=0.1, stop=1.0, num=10):
        # Instantiate model:
        model = RandomForest(
            n_estimators=1000, random_state=0, max_features=max_features
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Find the best parameters of a given model trained on a given dataset."
    )
    parser.add_argument(
        "model_type",
        metavar="model_type",
        type=str,
        help="An string defining the model type to use.",
    )
    parser.add_argument(
        "dataset",
        metavar="dataset",
        type=str,
        help="A string defining the name of the dataset to use.",
    )
